...
class HueCurvesFilter final {
public:
  explicit HueCurvesFilter(obs_data_t *settings, obs_source_t *source);
  ~HueCurvesFilter();
  ...
  [[nodiscard]] std::array<float, 256> histogram(int channel_index) const;
...
private:
  ...
  void open_editor();
  void ensure_preview_surfaces(uint32_t width, uint32_t height);
  void update_preview_from_target();
  ...
  gs_texrender_t *preview_texrender_ = nullptr;
  gs_stagesurf_t *preview_stage_ = nullptr;
  uint32_t preview_width_ = 0;
  uint32_t preview_height_ = 0;

  std::array<hue_curves::CurvePoints, 3> curves_ = hue_curves::default_curves();
  std::array<uint8_t, 1024> lut_data_ {};
  std::array<float, 256> hue_histogram_ {};
