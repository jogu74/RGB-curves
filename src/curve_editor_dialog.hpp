...
class CurveEditorDialog final : public QDialog {
  Q_OBJECT

public:
  explicit CurveEditorDialog(const std::array<rgb_curves::CurvePoints, 4> &initial_curves,
  ...
  [[nodiscard]] std::array<rgb_curves::CurvePoints, 4> curves() const;

signals:
  void curvesChanged(const std::array<rgb_curves::CurvePoints, 4> &curves);
...
private:
  ...
  void update_preview();
  void apply_curves(const std::array<rgb_curves::CurvePoints, 4> &curves);

  std::array<rgb_curves::CurvePoints, 4> curves_;
  std::array<rgb_curves::PreparedCurve, 4> prepared_curves_ {
    rgb_curves::prepare_curve(rgb_curves::default_curve()),
    rgb_curves::prepare_curve(rgb_curves::default_curve()),
    rgb_curves::prepare_curve(rgb_curves::default_curve()),
    rgb_curves::prepare_curve(rgb_curves::default_curve()),
  };
  ...
  std::array<QPushButton *, 4> channel_buttons_ {};
